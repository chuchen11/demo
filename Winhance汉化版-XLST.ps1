...
- 禁用 OneDrive 系统级功能`n`n确定要继续吗？" -Title "警告" -Buttons "YesNo" -Icon "Warning"
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
        ...
        try {
            ...
            Write-Status -Message "已创建计划任务，在启动时运行OneDriveRemoval.ps1。" -TargetScreen SoftAppsScreen
            Update-WPFControls

            # Prompt user about cleaning up their OneDrive data
            $cleanupMessage = "您是否要删除本地OneDrive文件夹和数据？`n`n" +
            "请注意：这将删除您计算机上的所有本地OneDrive数据。如果您之前已同步过文件，仍可以通过OneDrive.com访问这些文件。`n`n" +
            "此步骤是可选的，仅在需要时执行:`n" +
            "- 您确定您的文件已同步到OneDrive.com了吗？`n" +
            "- 不再需要您的一份本地副本的OneDrive文件`n" +
            "- 了解如何从 OneDrive.com 访问您的文件`n`n" +
            "此操作无法撤消。"

            $cleanupResult = Show-MessageBox -Message $cleanupMessage -Title "OneDrive 数据清理" -Buttons YesNo -Icon Warning

            if ($cleanupResult -eq [System.Windows.Forms.DialogResult]::Yes) {
                Write-Status -Message "清理 OneDrive 文件夹..." -TargetScreen SoftAppsScreen
                Update-WPFControls
            ...
            Show-MessageBox -Message "OneDrive 已成功从您的系统中移除。`n`n创建了一个启动任务以防止它重新安装。`n如果遇到问题，可以在任务计划程序中删除“OneDriveRemoval”任务。" -Title "成功" -Buttons OK -Icon Information
            Write-Status -Message "OneDrive 已成功移除！" -TargetScreen SoftAppsScreen
            Update-WPFControls
        }
        catch {
...
function Remove-SelectedBloatware {
    function Process-Child {
        ...
        if ($child -is [System.Windows.Controls.CheckBox] -and $child.IsChecked -eq $true) {
            if ($child.Content -eq "选择全部") {
                Write-Status "跳过“全选”复选框。"
                return
            }
    ...
    foreach ($child in $uniformGrid.Children) {
        if ($child -is [System.Windows.Controls.CheckBox] -and $child.IsChecked -eq $true) {
            if ($child.Content -eq "选择全部") { continue }
    ...
    if ($hasNormalApps.Value) {
        $result = Show-MessageBox -Message "您即将从系统中删除可能导致不稳定的应用程序。`n缺失的应用程序将必须单独重新安装。`n您确定要继续吗？" -Title "警告" -Buttons "YesNo" -Icon "Warning"
        ...
        if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
            ...
            foreach ($child in $uniformGrid.Children) {
                if ($child -is [System.Windows.Controls.CheckBox] -and $child.IsChecked -eq $true) {
                    if ($child.Content -eq "选择全部" -or $child.Tag -eq "Edge" -or $child.Tag -eq "OneDrive") { 
                        continue 
                    }
            ...
            Register-ScheduledTask -TaskName "Winhance\BloatRemoval" -Action (New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-File `"$scriptPath`"") -Trigger (New-ScheduledTaskTrigger -AtStartup) -User "SYSTEM" -RunLevel Highest -Settings (New-ScheduledTaskSettingsSet -DontStopIfGoingOnBatteries -AllowStartIfOnBatteries) -Force
            Write-Status "Scheduled task created to run BloatRemoval.ps1 at startup." -TargetScreen SoftAppsScreen

            Write-Status "已移除选定的应用程序。" -TargetScreen SoftAppsScreen
            Show-MessageBox -Message "已删除选定的应用程序。`n`n创建了一个启动任务以防止应用程序重新安装。如果您将来遇到自动应用程序删除问题，可以在任务计划程序中删除'BloatRemoval'任务。" -Title "成功" -Buttons "OK" -Icon "Information"
        }
        else {
          ...
          Margin="10,56,0,0">
            ...
            <StackPanel x:Name="SoftAppsScreen" Width="943" Height="550" HorizontalAlignment="Left" VerticalAlignment="Top" Margin="93,56,0,0" Visibility="Collapsed">
                ...
                <DockPanel HorizontalAlignment="Left" VerticalAlignment="Center">
                    ...
                    <StackPanel VerticalAlignment="Center">
                        <TextBlock Height="35" VerticalAlignment="Top" FontFamily="Microsoft YaHei UI" FontSize="28" FontWeight="Bold" Foreground="White" Text="软件 &amp; 应用" />
                        ...
                        <StackPanel Margin="10">
                            ...
                            <StackPanel>
                                <Border x:Name="InstallSoftwareHeader" Background="#202020" CornerRadius="5" Margin="0,5,0,5" Effect="{StaticResource ShadowEffect}">
                                    <DockPanel VerticalAlignment="Center" HorizontalAlignment="Stretch">
                                        <TextBlock Text="安装软件" HorizontalAlignment="Left" VerticalAlignment="Center" FontSize="18" FontWeight="Bold" Foreground="White" Padding="10" DockPanel.Dock="Left" />
                                ...
                                <Border Background="#202020" CornerRadius="5" Margin="5,0,5,5">
                                    <StackPanel x:Name="InstallSoftwareContent" Margin="0,10,0,10" Visibility="Collapsed">
                                        <Grid Margin="10,5">
                                            ...
                                            <TextBlock Text="应用商店" VerticalAlignment="Center" Foreground="White" FontSize="14" Margin="10,0,0,0" Grid.Column="0" />
                                            <Button Style="{StaticResource PrimaryButtonStyle}" Content="安装" Width="80" Height="30" HorizontalAlignment="Right" Grid.Column="1" x:Name="InstallStore" FontFamily="Microsoft YaHei UI" />
                                        </Grid>
                                        <Grid Margin="10,5">
                                            <Grid.ColumnDefinitions>
                                                <ColumnDefinition Width="*" />
                                            ...
                                            <Button Style="{StaticResource PrimaryButtonStyle}" Content="安装" Width="80" Height="30" HorizontalAlignment="Right" Grid.Column="1" x:Name="InstallUniGetUI" FontFamily="Microsoft YaHei UI" />
                                        </Grid>
                                        <Grid Margin="10,5">
                                            <Grid.ColumnDefinitions>
                                                <ColumnDefinition Width="*" />
                                            ...
                                            <TextBlock Text="Thorium 浏览器" VerticalAlignment="Center" Foreground="White" FontSize="14" Margin="10,0,0,0" Grid.Column="0" />
                                            <Button Style="{StaticResource PrimaryButtonStyle}" Content="安装" Width="80" Height="30" HorizontalAlignment="Right" Grid.Column="1" x:Name="InstallThorium" FontFamily="Microsoft YaHei UI" />
                                        </Grid>
                                        <Grid Margin="10,5">
                                            <Grid.ColumnDefinitions>
                                                <ColumnDefinition Width="*" />
                                            ...
                                            <Button Style="{StaticResource PrimaryButtonStyle}" Content="安装" Width="80" Height="30" HorizontalAlignment="Right" Grid.Column="1" x:Name="InstallFirefox" FontFamily="Microsoft YaHei UI" />
                                        </Grid>
                                        <Grid Margin="10,5">
                                            <Grid.ColumnDefinitions>
                                                <ColumnDefinition Width="*" />
                                            ...
                                            <Button Style="{StaticResource PrimaryButtonStyle}" Content="安装" Width="80" Height="30" HorizontalAlignment="Right" Grid.Column="1" x:Name="InstallChrome" FontFamily="Microsoft YaHei UI" />
                                        </Grid>
                                        <Grid Margin="10,5">
                                            <Grid.ColumnDefinitions>
                                                <ColumnDefinition Width="*" />
                                            ...
                                            <Button Style="{StaticResource PrimaryButtonStyle}" Content="安装" Width="80" Height="30" HorizontalAlignment="Right" Grid.Column="1" x:Name="InstallBrave" FontFamily="Microsoft YaHei UI" />
                                        </Grid>
                                        <Grid Margin="10,5">
                                            <Grid.ColumnDefinitions>
                                                <ColumnDefinition Width="*" />
                                            ...
                                            <Button Style="{StaticResource PrimaryButtonStyle}" Content="安装" Width="80" Height="30" HorizontalAlignment="Right" Grid.Column="1" x:Name="InstallEdge" FontFamily="Microsoft YaHei UI" />
                                        </Grid>
                                        <Grid Margin="10,5">
                                            <Grid.ColumnDefinitions>
                                                <ColumnDefinition Width="*" />
                                            ...
                                            <Button Style="{StaticResource PrimaryButtonStyle}" Content="安装" Width="80" Height="30" HorizontalAlignment="Right" Grid.Column="1" x:Name="InstallEdgeWebView" FontFamily="Microsoft YaHei UI" />
                                        </Grid>
                                        <Grid Margin="10,5">
                                            <Grid.ColumnDefinitions>
                                                <ColumnDefinition Width="*" />
                                            ...
                                            <Button Style="{StaticResource PrimaryButtonStyle}" Content="安装" Width="80" Height="30" HorizontalAlignment="Right" Grid.Column="1" x:Name="InstallOneDrive" FontFamily="Microsoft YaHei UI" />
                                        </Grid>
                                        <Grid Margin="10,5">
                                            <Grid.ColumnDefinitions>
                                                <ColumnDefinition Width="*" />
                                            ...
                                            <TextBlock Text="Xbox 应用" VerticalAlignment="Center" Foreground="White" FontSize="14" Margin="10,0,0,0" Grid.Column="0" />
                                            <Button Style="{StaticResource PrimaryButtonStyle}" Content="安装" Width="80" Height="30" HorizontalAlignment="Right" Grid.Column="1" x:Name="InstallXbox" FontFamily="Microsoft YaHei UI" />
                                        </Grid>
                                    </StackPanel>
                                </Border>
                            </StackPanel>
                            ...
                            <StackPanel>
                                <Border x:Name="RemoveAppsHeader" Background="#202020" CornerRadius="5" Margin="0,5,0,5" Effect="{StaticResource ShadowEffect}">
                                    <DockPanel VerticalAlignment="Center" HorizontalAlignment="Stretch">
                                        <TextBlock Text="移除 Windows 应用" HorizontalAlignment="Left" VerticalAlignment="Center" FontSize="18" FontWeight="Bold" Foreground="White" Padding="10" DockPanel.Dock="Left" />
                                        <TextBlock Text="&#xE70D;" FontFamily="Segoe MDL2 Assets" HorizontalAlignment="Right" VerticalAlignment="Center" FontSize="16" Foreground="White" Padding="10" DockPanel.Dock="Right" />
                                    </DockPanel>
                                </Border>
                                <Border Background="#202020" CornerRadius="5" Margin="5,0,5,5">
            ...
            <StackPanel x:Name="OptimizeScreen" Width="943" Height="550" HorizontalAlignment="Left" VerticalAlignment="Top" Margin="93,56,0,0" Visibility="Collapsed">
                ...
                <DockPanel HorizontalAlignment="Left" VerticalAlignment="Center">
                    <TextBlock Width="80" Height="70" Margin="0,0,0,0" DockPanel.Dock="Left" FontFamily="Segoe UI Emoji" FontSize="60" Foreground="White" Text="&#x1F680;"  LineHeight="70" LineStackingStrategy="BlockLineHeight" />
                    <StackPanel VerticalAlignment="Center">
                        <TextBlock Height="35" VerticalAlignment="Top" FontFamily="Microsoft YaHei UI" FontSize="28" FontWeight="Bold" Foreground="White" Text="优化" />
                        <DockPanel LastChildFill="False" Width="861">
                            ...
                            <Button x:Name="OptimizeDefaultsButton" DockPanel.Dock="Right" Style="{StaticResource PrimaryButtonStyle}" Content="默认" Width="80" Height="30" Margin="0,0,5,0"/>
                            <Button x:Name="OptimizeApplyButton" DockPanel.Dock="Right" Style="{StaticResource PrimaryButtonStyle}" Content="应用" Width="80" Height="30" Margin="0,0,10,0"/>
                        </DockPanel>
                    </StackPanel>
                </DockPanel>
                <!-- Main Content -->
                    ...
                    <ScrollViewer VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Disabled">
                        <StackPanel Margin="10">
                            <Border x:Name="WindowsSecurityHeaderBorder" Background="#202020" CornerRadius="5" Margin="0,5,0,5" Effect="{StaticResource ShadowEffect}">
                                <DockPanel VerticalAlignment="Center" HorizontalAlignment="Stretch">
                                    <TextBlock Text="Windows 安全设置" HorizontalAlignment="Left" VerticalAlignment="Center" FontSize="18" FontWeight="Bold" Foreground="White" Padding="10" DockPanel.Dock="Left" />
                                    <TextBlock Text="&#xE70D;" FontFamily="Segoe MDL2 Assets" HorizontalAlignment="Right" VerticalAlignment="Center" FontSize="16" Foreground="White" Padding="10" DockPanel.Dock="Right" />
                                </DockPanel>
                            </Border>
                                ...
                                Margin="25,5,0,0"/>
                                        ...
                                        <StackPanel Grid.Column="1" Margin="10,0">
                                            ...
                                            <Grid Margin="0,0,0,5">
                                                ...
                                                <TextBlock Text="低" Foreground="White" HorizontalAlignment="Left"/>
                                                <TextBlock Text="中等" Foreground="White" Grid.Column="1" HorizontalAlignment="Center"/>
                                                <TextBlock Text="高" Foreground="White" Grid.Column="2" HorizontalAlignment="Right"/>
                                            </Grid>
...
Margin="27,0,0,0"/>
                            ...
                            <Border Background="#202020" CornerRadius="5" Margin="5,5,5,5">
                                <StackPanel Orientation="Horizontal" Margin="10">
                                    <TextBlock Style="{StaticResource HelpIconStyle}" Margin="0,0,10,0">
                                        <TextBlock.ToolTip>
                                            <ToolTip Style="{StaticResource CustomTooltipStyle}" FontFamily="Microsoft YaHei UI">
                                                <TextBlock>
                                   禁用:
...
<LineBreak />将其他服务设置为允许的“手动”。
            ...
            <StackPanel x:Name="CustomizeScreen" Width="943" Height="550" HorizontalAlignment="Left" VerticalAlignment="Top" Margin="93,56,0,0" >
                ...
                <DockPanel HorizontalAlignment="Left" VerticalAlignment="Center">
                    <TextBlock Width="80" Height="70" Margin="0,0,0,0" DockPanel.Dock="Left" FontFamily="Segoe UI Emoji" FontSize="60" Foreground="White" Text="&#x1F3A8;"  LineHeight="70" LineStackingStrategy="BlockLineHeight" />
                    <StackPanel VerticalAlignment="Center">
                        <TextBlock Height="35" VerticalAlignment="Top" FontFamily="Microsoft YaHei UI" FontSize="28" FontWeight="Bold" Foreground="White" Text="自定义" />
                        <DockPanel LastChildFill="False" Width="861">
                            ...
                            <Button x:Name="CustomizeDefaultsButton" DockPanel.Dock="Right" Style="{StaticResource PrimaryButtonStyle}" Content="默认" Width="80" Height="30" Margin="0,0,5,0"/>
                            <Button x:Name="CustomizeApplyButton" DockPanel.Dock="Right" Style="{StaticResource PrimaryButtonStyle}" Content="应用" Width="80" Height="30" Margin="0,0,10,0"/>
                        </DockPanel>
                    </StackPanel>
                </DockPanel>
...
$uacSliderValue.Text = "当前: $(@("低","中等","高")[$uacSlider.Value])"
...
$CloseButton.Add_Click({
        # Define paths
        $configPath = Join-Path $env:ProgramFiles "Winhance\Config"
...
$tooltip.Content = "多余的软件: `n$BloatList$LegacyList"
...
function Update-SelectAllState {
    ...
    # Determine if all checkboxes (excluding "选择全部") are checked
    $allChecked = $true
    foreach ($chk in $checkBoxes) {
